...
public class FileAdapter extends RecyclerView.Adapter<FileViewHolder> {
    ...
    public void onBindViewHolder(@NonNull FileViewHolder holder, int position) {
        if(file.get(position).getName().toLowerCase().startsWith(".trashed")){
            holder.tvName.setText(file.get(position).getName().substring(20));
        }
        else{
        ...
        if(file.get(position).isDirectory()){
            ...
            holder.tvSize.setText(String.valueOf(items)+ " items");
            ...
            if(file.get(position).getName().toLowerCase().endsWith(".jpeg") || file.get(position).getName().toLowerCase().endsWith(".jpg") ||
            ...
            else if (file.get(position).getName().toLowerCase().endsWith(".mp3") || file.get(position).getName().toLowerCase().endsWith(".wav")) {
                holder.imgFile.setImageResource(R.drawable.ic_aud);
            }
            else if (file.get(position).getName().toLowerCase().endsWith(".mp4") || file.get(position).getName().toLowerCase().endsWith(".mkv")) {
                holder.imgFile.setImageResource(R.drawable.ic_vid);
            }
            else if (file.get(position).getName().toLowerCase().endsWith(".apk")) {
                holder.imgFile.setImageResource(R.drawable.ic_and);
            }
            else {
